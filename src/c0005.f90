...
!        write(output,*) l(il),m1(il)
           READ(ipot,*) fi(0:nmax,il)
!           write(output,*) fi(0:nmax,il)
!           write(output,'(200F15.6)') (fi(ir,il),ir=nmax,0,-1)
!           flush(output)
     enddo
  endif
...
!                 kk=j1+ka+il+parite+p6tab(mod(ka,6))
              ...
              enddo
           enddo
!           write(output,*)'nlev,parite=',nlev,parite
!                   write(*,*) 'nlev=',nlev
        if (nlev/=0) then
...
!                 write(*,*)'ptab   ',nlev,ptab(nlev),j1,ka
                    endif
                 enddo
              enddo
!              write(9,*)k2tab
           call V05(jtot,nlev,fl)
...
!                 call ijohnson_d(rmin,rmax,RP,nlev,npas,fl)
           ...
           if(nopen.EQ.0) then
!           write(output,*)'nlev,parite=  1   ',nlev,parite,nopen
           ...
           !  write(*,*)'Tr   '
           !  write(*,*) Tr
!           write(output,*)'sections partielles  Jtot =   ',jtot
           do i=1,nopen
              do j=1,nopen
                 j1=j1tab(i)
                 sigma(jjtab(i),jjtab(j))=sigma(jjtab(i),jjtab(j))+Tr(i,j)/(k2tab(i)*dfloat((2*j1+1)))
!                 som=Tr(i,j)/(k2tab(i)*dfloat((2*j1+1)))
!                 write(output,'(2I5,F19.6)')jjtab(i),jjtab(j),som
!                 sigma(jjtab(i),jjtab(j))=sigma(jjtab(i),jjtab(j))+som
              enddo
           enddo
...
do i=1,nlevo
           ...
           j1=j1tabs(i)
           ka=katabs(i)
write(output,2500) i,m,j1,ka, ktab(i)
...
enddo
  ...
  do n=1,nlevo
  do m=1,nlevo
!  write(output,*) m,n,sigma(jtab(m),jtab(n))
  enddo
  enddo
