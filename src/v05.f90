...
!
!
!$OMP PARALLEL DO PRIVATE(il, ll, mm1, ipar, fack, som1, som6, som5,omega,mm1p)
        do il=0,lamax
        ll=l(il)
        mm1=m1(il)
!              if(mm1==0)then
...
!                      facmm=1.d0
!              endif
        ipar=mod(iabs(-jtot+j1i+j1j+kai),2)
        fack=fpar(ipar)
        ...
        if((kaj-kai).GE.0)then
           ...
           mm1p=+mm1
              ...
              fl(i,j,il)=som1*som5*som6*faci*facj*fack*dsqrt(dfloat(2*ll+1)/(4.d0*pi))*facmi*facmj
           enddo
!$OMP END PARALLEL DO
!
!
