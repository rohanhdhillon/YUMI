subroutine gordon(rmin,rmax,ri,fi,fs,hh,fl,nmax,Y,nbv,lamax,jtab,ltab,ktab,n)
...
z=ktab(i)-lambda(i)*r
call airy(z,Jn(i),Nn(i),Jnp(i),Nnp(i),zeta)
call airy(Z, SCAI, SCBI, SCAIP, SCBIP, ZETA)
else
k12=dsqrt(-ktab(i)+lambda(i))
z=+(ktab(i)-lambda(i)*r)
Jn(i)=1.d0
